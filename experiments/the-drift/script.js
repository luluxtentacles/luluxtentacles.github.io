/* ==========================================================================
   ...
   direction the draw leaned. there is no third mode and there is no watch
   button: a draw you did not guess on is just the oracle breathing, it moves
   the stage and touches no ledger at all. you guess left or right, and that
   guess is spent on the NEXT draw the oracle produces. the guess is armed,
   in the browser, before the fetch goes out. nothing is stored on the server
   side, and nothing exists before the oracle answers.
   ...
   scoring: PEAR-style. the ONLY scored trials are the ones you guessed on:
   a hit is a draw that leaned the way you guessed. z = (2h - n) / sqrt(n)
   over the pooled guessed trials, one-tailed p in the guessed direction.
   unguessed draws are never logged, never scored, they are the stage's own
   breathing. local-fallback guesses are logged in their own ledger and are
   NEVER pooled into the quantum headline. the two sources are disclosed
   separately, always.
   ========================================================================== */
'use strict';
...
// ---------- THE MATH ----------
// binomial z for h hits in n guessed trials at p=0.5: (2h - n)/sqrt(n).
function binomZ(h, n) {
    if (n < 1) return null;
...
// one-tailed p in the guessed direction: the tail beyond |z|.
...
// a draw with no guess on it is not here at all: it was the oracle breathing.
const LEDGER_KEY = 'lulu_drift_v2';
let ledger = {
    q: { n: 0, h: 0, left: { n: 0, h: 0 }, right: { n: 0, h: 0 } },   // quantum draws, guessed
    f: { n: 0, h: 0, left: { n: 0, h: 0 }, right: { n: 0, h: 0 } },   // local-fallback guesses
    receipts: []                                                      // last 30 guessed trials, newest first
};
// old v1 saves carried a watch ledger; a v1 blob loads clean: its watch
// counts were pre-declared baseline and this page no longer scores any.
function loadLedger() {
    try {
...
// spent and the stage goes back to plain drifting. the commit hash is made
// the instant the guess is armed, before the next fetch goes out, so the
// receipt shows guess-then-draw in the order you can check.
// anything else the oracle produces is unaimed: it moves the stage, scores nothing.

let pendingGuess = null;        // 'left' | 'right' | null
...
async function armGuess(m) {
    ...
    pendingGuess = m;
    setBanner('armed: next breath leans ' + m.toUpperCase(),
...
// otherwise it is just the oracle moving: no ledger, no receipt, no score.
async function drawOnce() {
    const ts = Date.now();
    // the guess is snapshotted BEFORE the fetch goes out: anything armed while
    // a draw is already mid-flight waits for the NEXT one, so the covenant holds.
    ...
    const guess = vow ? vow.guess : null;        // the armed guess, set before the fetch
    const rec = guess ? {
        t: ts,
        mode: guess,
        bit: result.bit,
        src,
        hit: (guess === 'left' && result.bit === 0) || (guess === 'right' && result.bit === 1) ? 1 : 0,
        byte: result.byteHex,
        commit: vow.commit
    } : null;

    // the stage moves on every draw. only a guessed draw touches a ledger.
    if (rec) {
        const bin = src === 'q' ? ledger.q : ledger.f;
        bin.n++; bin[guess].n++;
        if (rec.hit === 1) { bin.h++; bin[guess].h++; }
    ...
    // needle target: pooled quantum z. fallback guesses move the grey needle only.
    ...
    gauge.flashSide = result.bit * 2 - 1;        // every draw: which way it leaned
    ...
    if (guess === null) {
        setBanner('the oracle leaned ' + sideWord(result.bit),
            src === 'q' ? 'unaimed breath. it scores nothing. guess left or right to spend the next one.' :
            'unaimed breath, from the local fall (oracle unreachable). it scores nothing.');
    } else if (rec.hit === 1) {
...
function updatePanel() {
    ...
    if (!q.n) {
        headline.textContent = 'no guesses answered yet. guess a side and the next breath decides it.';
        ...
        headline.textContent = 'past two sigma in your direction. either you are bending it, or you got lucky within one in ' + Math.round(1 / (pq * 100)) + '. keep guessing.';
    } else {
        headline.textContent = 'past two sigma in your direction. guess more before you believe it.';
    }

    // buttons: only two exist. each glows while its guess is armed and unspent.
    document.querySelectorAll('.vow-btn').forEach(b => {
        b.classList.toggle('active', b.dataset.mode === pendingGuess);
    });
}
...
function renderReceipts() {
    ...
    if (!ledger.receipts.length) {
        box.innerHTML = '<p class="foot-note" id="no-receipts">no guesses answered yet. the ledger starts when you spend your first guess.</p>';
        return;
    }
    const rows = ledger.receipts.map(r => {
        const t = new Date(r.t);
        const hhmm = t.toTimeString().slice(0, 8);
        const vow = r.mode === 'left' ? '\u2190 armed' : 'armed \u2192';
        const draw = r.bit === 0 ? '\u2190 left' : '\u2192 right';
        const mark = r.hit ? 'hole' : 'miss';
    ...
    box.innerHTML = '<table id="receipt-table"><thead><tr><th>at</th><th>guess</th><th>draw</th><th>src</th><th>verdict</th><th>byte 8</th></tr></thead><tbody>' + rows + '</tbody></table>';
}

// ---------- THE BUTTONS ----------
// two. guess left / guess right: arm it, and the NEXT draw is graded against
// it. there is no third button: a draw with no guess on it is the oracle
// breathing, it moves the stage and is never logged.

document.querySelectorAll('.vow-btn').forEach(b => {
    b.addEventListener('click', () => {
        armGuess(b.dataset.mode);
        updatePanel();
    });
});
...
document.getElementById('reset-ledger').addEventListener('click', () => {
    if (!confirm('burn the whole ledger? every guessed trial, both sources, all receipts.')) return;
    ledger = { q: { n: 0, h: 0, left: { n: 0, h: 0 }, right: { n: 0, h: 0 } },
        f: { n: 0, h: 0, left: { n: 0, h: 0 }, right: { n: 0, h: 0 } }, receipts: [] };
...
document.getElementById('export-ledger').addEventListener('click', () => {
    const out = {
        ...
        protocol: 'the oracle streams fresh draws continuously (an armed guess is spent BEFORE the draw it is graded on): arm left/right -> sha-256 commit shown in the receipt -> the NEXT 128-byte draw from the scrying relay (qrandom.io) is graded against the armed guess -> verdict bit = byte 8 & 1, then the guess is spent. draws with no guess armed move the stage and are never logged or scored. local-fallback guesses are tagged "f" and scored in their own ledger, never pooled.',
...
function receiptCountLine() {
    return ledger.receipts.length + ' receipts, both ledgers, all of it, as json.';
}
...
function drawGauge() {
    ...
    ctx.fillText('z = ' + (gauge.targetZ === 0 ? '\u2014' : (gauge.targetZ >= 0 ? '+' : '\u2212') + Math.abs(gauge.targetZ).toFixed(2) + '  (pooled, your guesses)'), 0, arm + 44);
