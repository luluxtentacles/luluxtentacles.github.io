...
m_era = ((hd >= pd.Timestamp("2026-03-20")) & (hd <= pd.Timestamp("2026-04-29"))).to_numpy()
ho = hs["Section"].map(orig_of).to_numpy()
asec_n = as_["Section"].astype(str).str[1:].astype(int)
h_cell = hs_d[m_era & (ho <= 1400)]
a_cell = ai_d[(asec_n <= 1400) | ((asec_n >= 2000) & (asec_n < 4000))]
report("humans Mar-Apr, base series only", h_cell)
report("ai, base family (their whole run)", a_cell)
diff = h_cell.mean() - a_cell.mean()
se = np.sqrt(h_cell.var(ddof=1)/len(h_cell) + a_cell.var(ddof=1)/len(a_cell))
print(f"\n  arm difference of means {diff:+.6f}  SE {se:.6f}  ->  z = {diff/se:+.2f}")
